...
def build_tasks(dataset_name: str, feats: dict) -> dict:
    ...
    if 'image_effnet' in feats: tasks['image_only'] = feats['image_effnet']
    elif 'image_precomp' in feats: tasks['image_only'] = feats['image_precomp']
...
def train_and_eval_single_task(
    ...
    if do_optuna and optuna_trials > 0:
        for name in tqdm(base_learners, desc="Optuna (per base)", leave=False):
            params, best = tune_base(name, Xtr, ytr, n_trials=optuna_trials,
